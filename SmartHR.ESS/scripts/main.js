...
function ShowPopup(text) {
    ...
    switch(response[0]) {
        ...
        case 'error': {
            ...
            break;
        case 'yesorno': {
                pcSubmit_014.SetHeaderText('INFORMATION');
                pcSubmit_014.SetHeaderImageUrl('images/info.png');
            };
            break;
    };

    if (response[0] == 'yesorno') {
        if (text.substring(response[0].length) == 'publicseminar') {
            pcYesOrNo.Show();
        }
        else {
            pcSubmit_014.Show();
