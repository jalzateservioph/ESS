    ...
    Private Sub LoadData()
        ...
        If (Not IsNull(objEmployee)) Then
            ...
            If (Not IsNull(SQLData) AndAlso Not IsPostBack) Then
                ...
                lblTransFrom.Text = GetXML(SQLData, KeyName:="OldCompanyNum")

                lblTransTo.Text = GetXML(SQLData, KeyName:="CompanyNum")

                lblTransFrom.Text &= " [ " & GetSQLField("select [CompanyName] from [Company] where ([CompanyNum] = '" & lblTransFrom.Text & "')", "CompanyName") & " ]"

                lblTransTo.Text &= " [ " & GetSQLField("select [CompanyName] from [Company] where ([CompanyNum] = '" & lblTransTo.Text & "')", "CompanyName") & " ]"
...
#Region " *** Web Form Events *** "
    ...
    Private Sub cpPage_Callback(ByVal source As Object, ByVal e As DevExpress.Web.ASPxCallback.CallbackEventArgs) Handles cpPage.Callback
        ...
        Dim bSaved As Boolean = ExecSQL("exec [ess.WFProc] '" & Session("LoggedOn").CompanyNum & "', '" & Session("LoggedOn").EmployeeNum & "', '" & CompanyNum & "', '" & EmployeeNum & "', " & PathID & ", 'Transfers', 'Transfers', '" & Values(0) & "', '" & GetXML(PathData, KeyName:="ActionType") & "', '" & Now.ToString("yyyy-MM-dd HH:mm:ss") & "'")
