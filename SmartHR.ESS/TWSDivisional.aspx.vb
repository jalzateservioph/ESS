...
Public Class TWSDivisional
    ...
    Private Sub btnBack_Click(sender As Object, e As System.EventArgs) Handles btnBack.Click
        Session("DTLDRDivision") = Nothing
        Session("DTTWSDivision") = Nothing
        Session("DTGRAND") = Nothing
        Response.Redirect("./twsreports.aspx")
    End Sub

    Public Sub Initialize()
        Session("DTLDRDivision") = GetSQLDT("SELECT RTRIM(LTRIM(Division)) Division, RTRIM(LTRIM(Department)) Department, RTRIM(LTRIM(Aspect)) Aspect, RTRIM(LTRIM(KPAName)) KPAName, PreviousYear, CurrentYear FROM [ess.TWSRawData] WHERE Aspect IN ('Leadership') AND CreatedBy = '" & Session("LoggedOn").UserID & "'")
        Session("DTTWSDivision") = GetSQLDT("SELECT RTRIM(LTRIM(Division)) Division, RTRIM(LTRIM(Department)) Department, RTRIM(LTRIM(Aspect)) Aspect, RTRIM(LTRIM(KPAName)) KPAName, PreviousYear, CurrentYear FROM [ess.TWSRawData] WHERE Aspect != ('Leadership') AND CreatedBy = '" & Session("LoggedOn").UserID & "'")
        ...
        Session("DTGRAND") = GetSQLDT("SELECT Department, ISNULL(AVG([PreviousYear]), 0) [PreviousYear], ISNULL(AVG([CurrentYear]), 0) [CurrentYear], ISNULL(AVG([CurrentYear]) - AVG([PreviousYear]), 0) [Diff] FROM (SELECT RTRIM(LTRIM(Department)) Department, RTRIM(LTRIM(Aspect)) Aspect, AVG(PreviousYear) [PreviousYear], AVG(CurrentYear) [CurrentYear] FROM [ess.TWSRawData] WHERE KPAName NOT IN ('CO', 'ESY') AND CreatedBy = '" & Session("LoggedOn").UserID & "' GROUP BY RTRIM(LTRIM(Department)), RTRIM(LTRIM(Aspect))) X GROUP BY Department")
    End Sub

    Public Sub LoadDataSource()
        LDRCorporate.DataSource = CType(Session("DTLDRDivision"), DataTable)
        LDRCorporate.DataBind()
        TWSCorporate.DataSource = CType(Session("DTTWSDivision"), DataTable)
        TWSCorporate.DataBind()
    End Sub

    Protected Sub btnExportTWS_Click(sender As Object, e As EventArgs) Handles btnExportTWS.Click
        Dim xFileName As String = "Departmental TWS Result - " & DateTime.Now.ToString("yyyyMMdd")
    ...
    Protected Sub btnExportLDR_Click(sender As Object, e As EventArgs) Handles btnExportLDR.Click
        Dim xFileName As String = "Departmental LDR Result  - " & DateTime.Now.ToString("yyyyMMdd")
    ...
    Protected Sub TWSCorporate_CustomCellValue(sender As Object, e As DevExpress.Web.ASPxPivotGrid.PivotCellValueEventArgs) Handles TWSCorporate.CustomCellValue
        If e.RowValueType = PivotGridValueType.GrandTotal Then
            ...
            If xDiv <> Nothing Then
                Dim xRow As DataRow() = xTable.[Select](String.Format("Department = '{0}'", xDiv.ToString()))
